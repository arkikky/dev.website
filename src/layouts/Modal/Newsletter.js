import React, { useState } from "react";
import Image from "next/image";

// @lib
import { SubmitForm } from "@lib/controller/HubSpot";

const NewsletterModal = ({ ipAddress }) => {
  const [isIpAddress, setIpAddress] = useState(ipAddress || []);
  const [isFirstname, setFirstname] = useState("");
  const [isEmail, setEmail] = useState("");

  // @Submit
  const handleSubmit = async (e) => {
    e.preventDefault();

    const btnNewsletter = document.querySelector(".mdlBtnNewsletter");
    const btnSuccessNewsletter = document.querySelector(
      "#mdlBtnSuccessNewsletter.mdlBtnSuccessNewsletter"
    );

    const isConfig = {
      fields: [
        {
          name: "firstname",
          value: isFirstname,
        },
        {
          name: "email",
          value: isEmail,
        },
      ],
      context: {
        pageUri: "https://coinfest.asia/",
        pageName: "Coinfest Asia 2024",
        ipAddress: isIpAddress.ip,
      },
    };

    const isKey = "9d5f70c2-2dc2-4f55-a8e4-791b0cc37642";

    const rs = await SubmitForm(isConfig, isKey);

    if (rs === true) {
      setFirstname("");
      setEmail("");

      btnNewsletter.click();
      btnSuccessNewsletter.click();
    }
  };

  return (
    <>
      <div
        id="mdlNewsletter"
        className="hs-overlay [--body-scroll:true] bg-black-900/[0.33] hidden fixed top-0 left-0 overflow-x-hidden overflow-y-auto h-full w-full z-[9999] opacity-0 hs-overlay-open:opacity-100 transition-all hs-overlay-open:duration-300"
        data-hs-overlay-backdrop-container="#bckdrpModalActve"
      >
        <div className="flex items-center justify-center fixed top-0 bottom-0 sm:inset-y-0 inset-x-0 mt-8 hs-overlay-open:mt-0 mx-auto px-4 sm:px-0 w-full max-w-full sm:max-w-[720px] lg:max-w-[835px] xl:max-w-[945px] opacity-0 hs-overlay-open:opacity-100 transition-all duration-300 ease-out">
          <div className="bg-white flex flex-col sm:flex-row flex-wrap rounded-xl sm:rounded-[18px] overflow-hidden relative py-0 w-full">
            <div className="flex-1 bg-gray-500 relative w-full">
              <Image
                className="absolute inset-y-0 inset-x-0 object-center object-cover mx-auto h-full w-full z-10"
                src={"/2024/assets/images/backdrop/ca2024BgNewsletter.png"}
                alt={`Coinfest Asia 2024 (Background Newsletter)`}
                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw"
                height={453}
                width={629}
                quality="87"
              />
            </div>
            <div className="flex-1 flex flex-col bg-whie pt-10 pb-6 sm:py-8 px-4 sm:px-8 w-full max-w-[406px]">
              <button
                className="hs-dropdown-toggle outline-none absolute top-4.5 bottom-auto left-auto right-4"
                aria-labelledby="mdlNewsletter"
                data-hs-overlay="#mdlNewsletter"
              >
                <svg
                  className="h-4 w-4"
                  viewBox="0 0 8 8"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M0.258206 1.00652C0.351976 0.912791 0.479126 0.860131 0.611706 0.860131C0.744296 0.860131 0.871447 0.912791 0.965207 1.00652L3.61171 3.65302L6.25822 1.00652C6.30432 0.958771 6.35952 0.920671 6.42052 0.894471C6.48152 0.868271 6.54712 0.854471 6.61352 0.853901C6.67992 0.853321 6.74572 0.865971 6.80722 0.891111C6.86862 0.916251 6.92442 0.953381 6.97142 1.00032C7.01832 1.04727 7.05552 1.1031 7.08062 1.16454C7.10572 1.22599 7.11842 1.29183 7.11782 1.35822C7.11722 1.42461 7.10342 1.49022 7.07722 1.55122C7.05102 1.61222 7.01292 1.6674 6.96522 1.71352L4.31871 4.36002L6.96522 7.00648C7.05632 7.10078 7.10672 7.22708 7.10552 7.35818C7.10442 7.48928 7.05182 7.61468 6.95912 7.70738C6.86642 7.80018 6.74102 7.85268 6.60992 7.85388C6.47882 7.85498 6.35252 7.80458 6.25822 7.71348L3.61171 5.06702L0.965207 7.71348C0.870907 7.80458 0.744606 7.85498 0.613506 7.85388C0.482406 7.85268 0.357007 7.80018 0.264297 7.70738C0.171597 7.61468 0.119017 7.48928 0.117877 7.35818C0.116737 7.22708 0.167126 7.10078 0.258206 7.00648L2.90471 4.36002L0.258206 1.71352C0.164476 1.61976 0.111816 1.4926 0.111816 1.36002C0.111816 1.22744 0.164476 1.10028 0.258206 1.00652Z"
                    fill="currentColor"
                  />
                </svg>
              </button>
              <div className="flex flex-col text-center">
                <h2 className="text-black-900 font-staraExtraBold text-xl sm:text-2xl uppercase text-start mt-2 sm:mt-4">
                  Be the First to Know When Tickets Open
                </h2>
                <form
                  id={"rstFormNewsletter"}
                  method="POST"
                  onSubmit={(e) => handleSubmit(e)}
                  className="flex flex-col mt-6"
                >
                  <div className="flex flex-col mb-4 last:mb-0">
                    <label
                      htmlFor="inputFirstname"
                      className="text-black-900 font-bevietnamPro text-base font-normal text-start mb-2"
                    >
                      First name
                    </label>
                    <input
                      type="text"
                      id="inputFirstname"
                      className="bg-white rounded-lg border border-solid border-[#E3E3E3] text-black-900 placeholder:text-[#9A9A9A] font-bevietnamPro text-sm font-normal capitalize py-5 px-4"
                      name="firstname"
                      placeholder="Michael"
                      value={isFirstname}
                      onChange={(e) => setFirstname(e.target.value)}
                      required
                    />
                  </div>
                  <div className="flex flex-col mb-4 last:mb-0">
                    <label
                      htmlFor="inputEmail"
                      className="text-black-900 font-bevietnamPro text-base font-normal text-start mb-2"
                    >
                      Email
                    </label>
                    <input
                      type="email"
                      id="inputEmail"
                      className="bg-white rounded-lg border border-solid border-[#E3E3E3] text-black-900 placeholder:text-[#9A9A9A] font-bevietnamPro text-sm font-normal lowercase py-5 px-4"
                      name="email"
                      placeholder="michaelzhao@company.com"
                      value={isEmail}
                      onChange={(e) => setEmail(e.target.value)}
                      required
                    />
                  </div>
                  <div>
                    <button
                      className={`bg-secondary2024 flex flex-col items-center justify-center rounded-[14px] text-white font-bevietnamPro text-base font-normal outline-none focus-visible:outline-none py-4 w-full transition duration-[0.3] ease-in-out`}
                      aria-label="Submit (Newsletter)"
                    >
                      Sign Up
                    </button>
                  </div>
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default NewsletterModal;
