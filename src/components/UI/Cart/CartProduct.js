  ...
  return (
    <>
      <div className="flex w-full flex-row justify-between space-x-3">
        <div
          className={twMerge(
            `flex h-[94px] w-full max-w-[245px] flex-row space-x-3 rounded-xl bg-primary px-3 py-2`,
            style[products?.documentId] || 'bg-primary'
          )}
        >
      ...
      <div className="flex flex-col items-end justify-between py-2">
        <span className="text-base font-medium text-black-900 sm:text-lg">
          {currencyConverter(
            getTotalProduct(
              products?.quantity,
