...
const SocialMentionCards = ({ attributes }) => {
  ...
  return (
    <>
      <Link
        ref={ref}
        className="relative flex flex-col rounded-lg border border-solid border-[#E3E3E3] bg-white px-6 py-6 outline-none transition-[border] duration-300 ease-in-out hocus:border-primary sm:border-2"
        href={
          attributes.url ??
          'https://twitter.com/OVioHQ/status/1699737577559875894?s=20'
        }
        title={`${publicRuntimeConfig?.siteAppName} Social Mentions - ${attributes?.name ?? 'Outlier Ventures'}`}
        target="_blank"
      ...
      >
        <div className="flex flex-row items-center justify-between">
          <div className="flex flex-row items-center justify-center">
            <div className="h-10 w-10 shrink-0">
              {attributes?.picture && (
                <Image
                  ...
                  alt={`${publicRuntimeConfig?.siteAppName} Social Mentions ${attributes?.name ?? 'Outlier Ventures'})`}
                  height={128}
                  width={128}
                  quality="87"
                />
            ...
            <div className="ml-3 flex flex-col items-start">
              <h3 className="flex flex-row items-center justify-center text-sm font-semibold text-black-900">
                {`${attributes?.name}` ?? 'Outlier Ventures'}
                {!attributes?.verified && (
                  <svg
              ...
              {attributes?.username && (
                <span className="text-sm font-normal text-gray-500">
                  {`@${attributes?.username ?? 'OVioHQ'}`}
                </span>
              )}
            </div>
          </div>
          <div className="h-6 w-6 shrink-0">
            {attributes?.category === 'Twitter' ? (
              <svg
                className="h-5 w-5"
                viewBox="0 0 24 24"
                fill="none"
        ...
        {attributes?.description && (
          <div className="relative mt-5 line-clamp-6 flex flex-col text-black-900">
            <ReactMarkdown className="ca25MarkdwnFrmattd ca25MarkdwnFrmattd_Mnsry text-balance text-sm">
              {attributes?.description ??
...
export default SocialMentionCards;
