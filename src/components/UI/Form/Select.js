...
const Select = ({
  ...
  return (
    <>
      <select
        id={id}
        className={`hsSlect form-select hidden ${
        ...
        data-hs-select={JSON.stringify({
          ...
          dropdownClasses: `mt-0 z-50 w-full max-h-72 px-2 ${
            withSearch === false ? "py-2" : "pt-0 pb-2"
          } bg-white border border-gray-200 rounded-xl overflow-hidden overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300`,
          optionClasses:
          ...
          extraMarkup: [
            '<div class="hidden hs-error:block absolute top-1/2 end-10 -translate-y-1/2"><svg class="shrink-0 h-4 w-4 sm:h-4.5 sm:w-4.5 text-red-500" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg></div>',
            '<div class="absolute block top-1/2 end-4 -translate-y-1/2"><svg class="shrink-0 h-4 w-4 sm:h-4.5 sm:w-4.5 text-gray-500 " xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7 15 5 5 5-5"/><path d="m7 9 5-5 5 5"/></svg></div>',
          ],
        })}
        {...config}
      >
