...
const PeopleSaying = ({ attributes }) => {
  const getCategory = attributes ? attributes.category : "Twitter";
  const getVerified = attributes ? attributes.verified : false;
  const getPicture = attributes
    ? publicRuntimeConfig.caUpload + attributes.picture.data.attributes.url
    : "/assets/images/people-saying/picture/caPeopleSayingPicture-OutlierVentures.png";
  ...
  return (
    <>
      <Link
        className="bg-white flex flex-col border border-solid border-[#E3E3E3] hocus:border-primary rounded-lg outline-none relative py-6 px-6 transition duration-300 ease-in-out"
        href={getUrl}
        target="_blank"
      ...
      >
        <div className="flex flex-row items-center justify-between">
          <div className="flex flex-row items-center justify-center">
            <div className="min-w-[40px] w-max">
              <Image
                className="object-cover object-center rounded-full overflow-hidden h-10 w-10"
                src={getPicture}
                alt={`Coinfest Asia 2024 by Coinvestasi (${getName} - People Saying)`}
                height={128}
                width={128}
                quality="87"
              />
            ...
            <div className="flex flex-col items-start ml-2">
              <h3 className="flex flex-row items-center justify-center text-black-900 font-bevietnamPro text-sm font-semibold">
                {getName}
                {getVerified === true && (
                  <svg
              ...
              {getUsername && (
                <span className="text-black-900/70 font-bevietnamPro text-sm font-normal">
                  {`@${getUsername}`}
                </span>
              )}
            </div>
        ...
        {getDesc && (
          <article className="artcleMarkdown flex flex-col text-black-900 font-bevietnamPro line-clamp-6 relative mt-4">
            <ReactMarkdown>{getDesc}</ReactMarkdown>
          </article>
        )}
      </Link>
    </>
