...
const Agenda = ({ day1, day2 }) => {
  ...
  const isTabDateActived = async (e, days, stage) => {
    ...
    gsap.to(window, {
      duration: 1.5,
      scrollTo: { y: "#ca2024TabsAgendaList", offsetY: "-60" },
      ease: "power2",
    });
  ...
  useEffect(() => {
    if (searchParams) {
      if (isTabs === "mainStage") {
        const gtAgendaTabs = document.querySelector("#ca2024AgendaTabs_Main");
      ...
      } else if (isTabs === "alphaStage") {
        const gtAgendaTabs = document.querySelector(
          "#ca2024AgendaTabs_AlphaStage",
        );
      ...
      <main className="relative pt-[146px]">
        <Container className={"px-0 pb-20 sm:pb-0"}>
          <div className="flex flex-col items-start justify-start px-4 pr-0 sm:px-0">
            <h1
              id="ca2024TabsAgendaList"
              className="font-staraExtraBold text-[32px] uppercase leading-[40px] text-black-900 sm:text-[58px] sm:leading-[74px] lg:text-[80px] lg:leading-[90px] xl:text-[72px] xl:leading-[86px] 2xl:text-[80px] 2xl:leading-[90px]"
            >
              Agenda {isTabs}
            </h1>
                  ...
                  {isDay1?.map((gtRsltItems, i) => (
                    <>
                      <div className="relative w-full" key={i}>
                        <div className="sticky bottom-auto top-[177px] z-40 w-full border-b border-[#D6D6D6] bg-[#EEEEEE] px-3 py-3 sm:top-[209px] sm:px-4 sm:py-4 lg:px-4 lg:py-4">
